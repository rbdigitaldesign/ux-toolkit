  ...
  Object.entries(model).forEach(([cat, info]) => {
    ...
    fs.innerHTML = `
      ...
      ${info.checklist.map(item =>
        `<label><input type="checkbox" data-cat="${cat}" data-item="${item}" /> ${item}</label>`
      ).join("")}
      <label style="margin-top:1em;">Overall score for '${cat}'</label>
      <div class="slider-container">
        <input type="range" min="0" max="3" value="0" class="slider" data-label="${cat}" />
        <div class="ticks">
...
function generateReport() {
  ...
  document.querySelectorAll('.slider').forEach(s => {
    ...
    reportData[c] = reportData[c] || { scores: [], selected: [], avg: 0 };
    reportData[c].scores.push(+s.value);
  });
...
function downloadPDF() {
  ...
  logo.onload = () => {
    ...
    Object.entries(data).forEach(([cat,info]) => {
      ...
      y += 7;
      doc.setFontSize(11).text(`Observed ${info.selected.length} of ${model[cat].checklist.length} items:`, 14, y);
      y += 6;
    ...
    doc.setFontSize(11);
    doc.splitTextToSize(meta.comments, pageW - 28).forEach(line => {
      if (y > pageH - 20) { doc.addPage(); y = 20; }
